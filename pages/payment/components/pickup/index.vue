			...
			<view class="main">
				...
				<view class="main_content">
					...
					<view v-if="showModel === 2">
						...
						<text class="address">{{ info.prov + info.city + info.area + info.street + info.address }}</text>
					</view>
			...
			address: {
				handler(newValue, oldValue) {
					if(this.address.user != '') {
						this.showModel = 2
						this.info.user = this.address.user
						this.info.phone = this.address.phone
						this.info.address = this.address.address
					}
				},
				deep: true
			},
					...
					success: (res) => {
						...
						try{
							if (res.resultStatus == 9000){
								...
								this.info.street = res.result.street
								this.info.address = res.result.address
								this.showModel = 2
								this.$emit('update:address', this.info)
							}
						}catch(e){
							console.log(e)
						}
